...
{
    public class MaintenanceRequestRepository : GenericRepository<MaintenanceRequest>
    {
        public MaintenanceRequestRepository()
        {
        }
        public MaintenanceRequestRepository(RentEaseContext context) => _context = context;
        public async Task<PagedResult<MaintenanceRequest>> GetAllAsync(bool? status, int page, int pageSize)
        {
            IQueryable<MaintenanceRequest> query = _context.Set<MaintenanceRequest>();

            Expression<Func<MaintenanceRequest, bool>> filter = a =>
                (!status.HasValue || a.Status == status.Value);

            return await GetPagedAsync(filter, null, page, pageSize);
        }
        public async Task<PagedResult<MaintenanceRequest>> GetAllForAptAsync(
